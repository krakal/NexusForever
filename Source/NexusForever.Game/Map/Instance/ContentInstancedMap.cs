...
namespace NexusForever.Game.Map.Instance
{
    public class ContentInstancedMap<T> : InstancedMap<T> where T : class, IContentMapInstance
    {
        ...
        {
            IMatch match = matchManager.GetMatchCharacter(player.Identity.CharacterId).Match;
            if (match != null)
            {
            ...
            IMapLock soloMapLock = mapLockManager.GetSoloLock(player.Identity.CharacterId, Entry.Id);
            return soloMapLock ?? mapLockManager.CreateSoloLock(player.Identity.CharacterId, Entry.Id);
        }
