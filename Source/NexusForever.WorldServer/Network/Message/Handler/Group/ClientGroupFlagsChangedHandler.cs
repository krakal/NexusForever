...
namespace NexusForever.WorldServer.Network.Message.Handler.Group
{
    public class ClientGroupFlagsChangedHandler : IMessageHandler<IWorldSession, ClientGroupFlagsChanged>
    {
        ...
        {
            GroupHandler.AssertGroupId(session, clientGroupFlagsChanged.GroupId);
            GroupHandler.AssertGroupLeader(session, clientGroupFlagsChanged.GroupId);

            IGroup group = GroupManager.Instance.GetGroupById(clientGroupFlagsChanged.GroupId);
            if (group == null)
            {
                GroupHandler.SendGroupResult(session, GroupResult.GroupNotFound, clientGroupFlagsChanged.GroupId, session.Player.Name);
                return;
            }
