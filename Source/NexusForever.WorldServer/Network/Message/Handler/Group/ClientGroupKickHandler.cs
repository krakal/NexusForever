...
namespace NexusForever.WorldServer.Network.Message.Handler.Group
{
    public class ClientGroupKickHandler : IMessageHandler<IWorldSession, ClientGroupKick>
    {
        ...
        {
            GroupHandler.AssertGroupId(session, kick.GroupId);
            GroupHandler.AssertPermission(session, kick.GroupId, GroupMemberInfoFlags.CanKick);

            IGroup group = GroupManager.Instance.GetGroupById(kick.GroupId);
            if (group == null)
            {
                GroupHandler.SendGroupResult(session, GroupResult.GroupNotFound, kick.GroupId, session.Player.Name);
                return;
            }
