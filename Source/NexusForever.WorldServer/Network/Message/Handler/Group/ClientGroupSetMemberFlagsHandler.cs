...
namespace NexusForever.WorldServer.Network.Message.Handler.Group
{
    public class ClientGroupSetMemberFlagsHandler : IMessageHandler<IWorldSession, ClientGroupSetMemberFlags>
    {
        ...
        {
            GroupHandler.AssertGroupId(session, clientSetMemberFlags.GroupId);

            IGroup group = GroupManager.Instance.GetGroupById(clientSetMemberFlags.GroupId);
            if (group == null)
            {
                GroupHandler.SendGroupResult(session, GroupResult.GroupNotFound, clientSetMemberFlags.GroupId, session.Player.Name);
                return;
            }
